...
export const syncLocationsDBOnLocationEvent = async () => {
  ...
  await lock.acquire('syncLocationsDBOnLocationEvent', async (done) => {
    try {
      ...
      await saveSampleForQAAllPoints(locations);
...
const saveSampleForQAAllPoints = (locations: Sample[]) => new Promise(async (resolve) => {
  try {
    const allPointsForQA = JSON.parse(await AsyncStorage.getItem(ALL_POINTS_QA) || '[]');

    await AsyncStorage.setItem(ALL_POINTS_QA, JSON.stringify([...allPointsForQA, ...locations.map(location => ({
      lat: location.coords.latitude,
      long: location.coords.longitude,
      accuracy: location.coords.accuracy,
      startTime: location.timestamp,

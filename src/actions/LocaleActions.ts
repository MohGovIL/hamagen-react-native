...
export const initLocale = () => async (dispatch: any) => {
  try {
    ...
    let activeLocale: 'he'|'iw'|'en'|'ar'|'am'|'ti'|'ru'|'fr' = (await AsyncStorage.getItem(CURRENT_LOCALE) || locale).substr(0, 2);
...
export const changeLocale = (locale: 'he'|'en'|'ar'|'am'|'ti'|'ru'|'fr') => async (dispatch: any) => {
  try {
